...
export function ResearchAnalyticsDashboard() {
  ...
  return <section aria-labelledby="analytics-title">
    ...
    <div className="mt-7 grid gap-3 sm:grid-cols-2 xl:grid-cols-4" aria-live="polite">
      ...
      {data.ttp && <Summary icon={Cpu} title="Optional alternate change service" value={analyticsLabel(data.ttp.service_status)} note={`${data.ttp.inference_count} inference · ${data.ttp.fallback_count} fallback`}/>}
    ...
    {data.ttp && <Panel className="mt-6" title="Alternate change-service operations" icon={Cpu} note="Process-local counters and validated service health only; no image pixels, filenames, or query text are retained.">
