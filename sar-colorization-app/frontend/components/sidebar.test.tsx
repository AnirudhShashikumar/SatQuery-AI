...
describe("SatQuery sidebar route safety", () => {
  it("exposes the complete grouped product navigation in the intended order", () => {
    ...
    expect(primaryNavigation.map(item => item.label)).toEqual([
      ...
      "Single Image", "Optical + SAR", "Bi-temporal", "Grounding", "Mission & Sensor Comparison",
      "Reports", "Benchmarks", "Demo Gallery", "Research Analytics", "SIH Compliance",
  ...
  it("keeps restored destinations separate from accidental route reuse", () => {
    ...
    expect(href("Mission & Sensor Comparison")).not.toBe(href("Reports"));
  ...
  it("keeps technical navigation available but collapsed by default", async () => {
    await renderSidebar();
    const disclosures = Array.from(container.querySelectorAll<HTMLDetailsElement>(".sat-nav-disclosure"));
    expect(disclosures).toHaveLength(3);
    expect(disclosures.map(item => item.querySelector("summary")?.textContent)).toEqual(expect.arrayContaining(["Models", "Developer", "System"]));
    expect(disclosures.every(item => !item.open)).toBe(true);
    const models = disclosures.find(item => item.textContent?.includes("SatQuery Vision Encoder"))!;
    await act(async () => { models.querySelector("summary")!.dispatchEvent(new MouseEvent("click", { bubbles: true })); await Promise.resolve(); });
    expect(models.open).toBe(true);
    expect(models.querySelector('a[aria-label="SatQuery Vision Encoder"]')).toBeTruthy();
  });
