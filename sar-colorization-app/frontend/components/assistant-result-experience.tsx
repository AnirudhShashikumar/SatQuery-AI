...
import {
  Check, CheckCircle2, ChevronDown, ChevronUp, Columns2,
...
import { EvidenceSources } from "@/components/satquery/evidence-sources";
import { agentPreviewUrl } from "@/services/api";
import type { AgentResponse, ChangeAnalysisResponse, SemanticChangeSummary } from "@/types/agent";
import { changeEngineName, presentedSpecialists, titleCase, workflowLabel } from "@/lib/scientific-presentation";
...
type EvidenceProduct = {
  sourceRole?: "optical" | "sar";
  label: string;
  url: string;
  description: string;
  contribution: "Primary Evidence" | "Supporting Evidence";
};
...
function addEvidence(items: EvidenceProduct[], label: string, url: string | null | undefined, description: string, contribution: EvidenceProduct["contribution"] = "Supporting Evidence", sourceRole?: "optical" | "sar") {
  if (url && !items.some(item => item.url === url)) items.push({ label, url, description, contribution, sourceRole });
}

export function evidenceProducts(result: AgentResponse, change?: ChangeAnalysisResponse): EvidenceProduct[] {
  ...
  const translatedProduct = authoritativeTranslatedEvidence(result);

  if (cross) {
    const observations = [result.primary_image_metadata, result.secondary_image_metadata];
    for (const role of ["optical", "sar"] as const) {
      const product = cross.evidence_products?.find(item => item.evidence_type === "source" && item.source_role === role);
      const metadata = observations.find(item => item?.observation_role === role);
      // Legacy records use explicit named previews, never primary/secondary position.
      const url = product ? product.reference : metadata?.preview_url ?? cross.previews[role];
      addEvidence(items, role === "optical" ? "Optical source" : "SAR source", url, product?.description ?? "Source observation preview; role retained by provenance.", "Primary Evidence", role);
    }
    cross.evidence_products?.filter(item => item.evidence_type !== "source").forEach(item => addEvidence(items, item.label, item.reference, item.description ?? "Source-linked candidate evidence.", "Primary Evidence"));
  } else {
    addEvidence(items, change ? "Before Observation" : "Original image", change?.previews.before ?? result.primary_image_metadata?.preview_url, "Source-image preview used by the workflow.", "Primary Evidence");
    addEvidence(items, change ? "After Observation" : "Secondary image", change?.previews.after ?? result.secondary_image_metadata?.preview_url, "Second observation supplied to the workflow.", "Primary Evidence");
  }

  if (grounding?.annotated_preview_url) addEvidence(items, "Grounding overlay", grounding.annotated_preview_url, "Accepted model-produced localization boxes over the source image.", "Primary Evidence");
  sarProducts.forEach(product => addEvidence(items, product.label, product.reference, product.description, product.type.includes("overlay") || product.type.includes("mask") ? "Primary Evidence" : "Supporting Evidence"));
  addEvidence(items, translatedProduct?.label ?? "Generated optical-like representation", translatedProduct?.reference, translatedProduct?.description ?? translated?.disclosure ?? "Learned optical-like representation; not optical ground truth.", "Supporting Evidence");
  if (vqa) {
    addEvidence(items, "Combined evidence overlay", vqa.previews.combined_overlay, "Combined deterministic support layers used by the controlled answer.", "Primary Evidence");
    ...
    maps.forEach(([name, url, description], index) => addEvidence(items, name, url, description, index === 0 && Boolean(target) ? "Primary Evidence" : "Supporting Evidence"));
  }
  if (change) {
    ...
    if (change.change_engine?.mode === "hybrid" || change.change_engine?.mode === "ttp") {
      addEvidence(items, "Learned Change Mask", change.previews.ttp_mask ?? change.previews.mask, `Primary ${engineName} model-generated binary change prediction; not ground truth.`, "Primary Evidence");
      addEvidence(items, "Deterministic Difference", change.previews.deterministic_mask, "Independent normalized-difference supporting evidence.");
      ...
      addEvidence(items, `${engineName} overlay`, change.previews.ttp_overlay ?? change.previews.overlay, `${engineName} learned change mask overlaid on the later source observation.`, "Primary Evidence");
      addEvidence(items, "Top Changed Regions", change.previews.top_regions, "Numbered outlines of real changed components; built-up-like labels remain heuristic.", "Primary Evidence");
    } else {
      addEvidence(items, "Deterministic Difference", change.previews.deterministic_mask ?? change.previews.mask, "Thresholded and morphologically cleaned deterministic change mask.", "Primary Evidence");
      addEvidence(items, "Deterministic overlay", change.previews.deterministic_overlay ?? change.previews.overlay, "Deterministic change evidence overlaid on the later observation.", "Primary Evidence");
    }
  }
  if (cross) {
    addEvidence(items, "Joint evidence overlay", cross.previews.joint_overlay, "Optical and SAR candidate evidence combined on the analysis grid.", "Primary Evidence");
    addEvidence(items, "Joint evidence", cross.previews.joint_evidence, "Shared candidate support across both modalities.", "Primary Evidence");
...
function metrics(result: AgentResponse, change?: ChangeAnalysisResponse): Metric[] {
  ...
  if (change?.statistics) {
    ...
    if (change.change_engine?.mode === "hybrid" || change.change_engine?.mode === "ttp") {
      const engineName = changeEngineName(change.change_engine, change.ttp_result);
      output.push(
        { label: `${engineName} changed area`, value: percent(change.ttp_result?.changed_percentage, 3), note: "Primary learned mask" },
        ...
        { label: `${engineName} regions`, value: (change.ttp_result?.region_count ?? stats.number_of_regions).toLocaleString() },
        { label: `${engineName} runtime`, value: change.ttp_result?.runtime_ms == null ? "Unavailable" : `${change.ttp_result.runtime_ms.toLocaleString()} ms` },
      );
    } else output.push(
...
type LimitationGroup = "Data" | "Model" | "Interpretation";

function groupedLimitations(result: AgentResponse, change?: ChangeAnalysisResponse): Record<LimitationGroup, string[]> {
  const values = [
    ...result.warnings,
    ...
    ...(result.sar_translated_optical_analysis?.limitations ?? []),
    ...(result.sar_translated_optical_analysis?.warnings ?? []),
    ...(result.ttp_result?.limitations ?? []),
    ...(result.ttp_result?.warnings ?? []),
    ...(change?.ttp_result?.limitations ?? []),
    ...(change?.ttp_result?.warnings ?? []),
    ...(change?.warnings ?? []),
    ...(result.semantic_change_summary?.caveats ?? change?.semantic_change_summary?.caveats ?? []),
  ...
  values.push("Model output is not ground truth", "Confidence is not a calibrated probability", "Semantic change cause is not inferred without supporting evidence");
  const output: Record<LimitationGroup, string[]> = { Data: [], Model: [], Interpretation: [] };
  Array.from(new Set(values.filter(Boolean))).forEach(value => {
    const group: LimitationGroup = /georef|crs|preview|input|band|align|registration|sensor|image|resolution|nodata|date/i.test(value)
      ? "Data"
      : /model|trained|training|levir|checkpoint|generaliz|domain|architecture|learned/i.test(value)
        ? "Model"
        : "Interpretation";
...
function timelineLabel(tool: string) {
  ...
  if (lower.includes("changerex")) return titleCase(tool.replace(/^rs_/, "")).replace(/Changerex/g, "ChangerEx");
  if (lower.includes("rsvqa")) return titleCase(tool.replace(/^rs_/, "")).replace(/Rsvqa/g, "RSVQA");
  if (lower.includes("sve")) return titleCase(tool.replace(/^rs_/, "")).replace(/Sve/g, "SVE");
...
type TimelineGroup = "Input" | "Validation" | "Model Inference" | "Evidence Fusion" | "Response";

function timelineGroup(tool: string): TimelineGroup {
  const lower = tool.toLowerCase();
  if (/upload|ingest|metadata|modality|input/.test(lower)) return "Input";
  if (/valid|compatib|align|rout|classif|eligib|prepar/.test(lower)) return "Validation";
  if (/evidence|fusion|agreement|comparison|overlay|preview|heatmap/.test(lower)) return "Evidence Fusion";
  if (/answer|response|report|compose|summary/.test(lower)) return "Response";
...
function EvidenceWorkbench({ products }: { products: EvidenceProduct[] }) {
  ...
  const [mode, setMode] = useState<"layer" | "overlay" | "split">(products.length > 1 && !products.some(product => product.sourceRole) ? "overlay" : "layer");
  ...
  const viewer = <div className="relative grid min-h-[320px] flex-1 place-items-center overflow-hidden rounded-2xl bg-slate-950/80 sm:min-h-[430px]" aria-label="Evidence image viewer">
    ...
    <span className="absolute left-3 top-3 rounded-lg bg-black/70 px-2.5 py-1 text-xs font-medium text-white">{mode === "layer" ? active.label : `${base.label} + ${active.label} · Visual ${mode === "overlay" ? "blend" : "split"}`}</span>
  </div>;
...
function ConfidenceDisclosure({ reason }: { reason: string }) {
  return <details className="result-confidence-explanation"><summary>What determines confidence?</summary><div><p>Confidence is derived from the evidence published for this workflow:</p><ul><li>Input quality</li><li>Workflow compatibility</li><li>Evidence agreement</li><li>Specialist availability</li><li>Model-specific confidence</li></ul><p><strong>This is not a calibrated probability.</strong> {reason}</p></div></details>;
}

function ChangeScientificSummary({ result, change }: { result: AgentResponse; change?: ChangeAnalysisResponse }) {
  const stats = change?.statistics;
  const engine = result.change_engine ?? change?.change_engine;
  const learned = result.ttp_result ?? change?.ttp_result;
  if (!stats || !engine) return null;
  const engineName = changeEngineName(engine, learned);
  const largest = stats.regions.length ? stats.regions.reduce((best, region) => region.area_pixels > best.area_pixels ? region : best, stats.regions[0]) : null;
  const centerX = largest ? (largest.bounding_box.left + largest.bounding_box.right) / 2 : null;
  const centerY = largest ? (largest.bounding_box.top + largest.bounding_box.bottom) / 2 : null;
  const horizontal = centerX == null ? null : centerX < stats.analysis_width / 3 ? "west" : centerX > stats.analysis_width * 2 / 3 ? "east" : "central";
  const vertical = centerY == null ? null : centerY < stats.analysis_height / 3 ? "north" : centerY > stats.analysis_height * 2 / 3 ? "south" : "central";
  const location = horizontal && vertical ? (horizontal === "central" && vertical === "central" ? "central area" : `${vertical === "central" ? "" : `${vertical}-`}${horizontal === "central" ? "central" : horizontal} area`) : null;
  const comparison = result.mask_comparison ?? change?.mask_comparison;
  const lowAgreement = comparison?.iou != null && comparison.iou < .35;
  const semantic = result.semantic_change_summary ?? change?.semantic_change_summary;
  return <div className="result-change-summary">
    <article><h3>Overview</h3><p>{semantic?.expanded_answer ?? <>{engine.fallback_used ? "The deterministic change analyzer completed the workflow because the learned specialist was unavailable." : `${engineName} identified ${stats.percentage_changed.toFixed(3)}% of the analysis grid as changed.`} {stats.number_of_regions ? `${stats.number_of_regions.toLocaleString()} connected region${stats.number_of_regions === 1 ? " was" : "s were"} reported.` : "No connected change regions were reported."}</>}</p></article>
    <article><h3>Largest detected region</h3><p>{semantic?.dominant_location ? `The largest detected change is centered in the ${titleCase(semantic.dominant_location)} part of the image.` : largest && location ? `The largest contiguous region is concentrated in the ${location} and occupies approximately ${largest.percentage_of_image.toFixed(3)}% of the image.` : "No largest-region location can be stated from the published geometry."}</p></article>
    <article><h3>Likely change type</h3><p>{semantic?.likely_change_type && semantic.likely_change_type !== "unknown_change" ? `${titleCase(semantic.likely_change_type)} · ${titleCase(semantic.evidence_strength)} evidence. This is an evidence-gated interpretation, not a confirmed land-cover label.` : "Unknown semantic change. The available evidence does not support a reliable land-cover transition label."}</p></article>
    <article><h3>Evidence interpretation</h3><p>The learned detector identifies patterns associated with change in its training data. The deterministic engine measures low-level normalized pixel differences. Scene-level semantic evidence is advisory and does not localize pixels.</p>{(lowAgreement || semantic?.visual_structural_disagreement) && <div className="result-disagreement-warning"><strong>Low agreement between structural and visual-difference evidence.</strong><span>Possible contributors include seasonal variation, illumination, sensor differences, registration error, or model uncertainty.</span></div>}</article>
  </div>;
}

function SemanticChangePanel({ summary }: { summary: SemanticChangeSummary }) {
  const builtUp = summary.built_up_assessment;
  return <section className="result-section" aria-labelledby="semantic-change-heading">
    <div className="result-section-heading"><div><p className="result-section-number">WHAT CHANGED</p><h2 id="semantic-change-heading">Human-readable interpretation</h2></div><p>Generated locally from structured, evidence-gated facts.</p></div>
    <article className="rounded-2xl border border-sky-300/20 bg-sky-300/[.055] p-5 sm:p-6">
      <p className="max-w-5xl text-lg font-medium leading-8 text-zinc-100 sm:text-xl">{summary.expanded_answer}</p>
      <dl className="mt-5 grid gap-3 sm:grid-cols-3"><div className="rounded-xl border border-white/[.08] bg-black/10 p-3"><dt className="text-[10px] uppercase tracking-[.14em] text-zinc-500">Likely change</dt><dd className="mt-1 text-sm font-semibold text-zinc-200">{titleCase(summary.likely_change_type ?? "unknown semantic change")}</dd></div><div className="rounded-xl border border-white/[.08] bg-black/10 p-3"><dt className="text-[10px] uppercase tracking-[.14em] text-zinc-500">Where</dt><dd className="mt-1 text-sm font-semibold text-zinc-200">{summary.dominant_location ? titleCase(summary.dominant_location) : "No dominant location"}</dd></div><div className="rounded-xl border border-white/[.08] bg-black/10 p-3"><dt className="text-[10px] uppercase tracking-[.14em] text-zinc-500">Evidence</dt><dd className="mt-1 text-sm font-semibold text-zinc-200">{titleCase(summary.evidence_strength)} · {summary.supporting_facts.length} supporting signal{summary.supporting_facts.length === 1 ? "" : "s"}</dd></div></dl>
      {summary.supporting_facts.length > 0 && <ul className="mt-5 grid gap-2 text-sm leading-6 text-zinc-400 md:grid-cols-2">{summary.supporting_facts.slice(0, 4).map(fact => <li key={fact} className="flex gap-2"><CheckCircle2 size={15} className="mt-1 shrink-0 text-sky-300"/><span>{fact}</span></li>)}</ul>}
      {builtUp && builtUp.regions.length > 0 && <div className="mt-5"><p className="text-[10px] font-semibold uppercase tracking-[.14em] text-zinc-500">Top changed regions</p><div className="mt-3 grid gap-3 md:grid-cols-3">{builtUp.regions.map((region, index) => <article key={region.region_id} className="rounded-xl border border-white/[.08] bg-black/15 p-4"><p className="text-[10px] font-semibold uppercase tracking-[.14em] text-sky-300">Region {index + 1} · {titleCase(region.relative_location)}</p><p className="mt-2 text-sm font-semibold text-zinc-200">{titleCase(region.change_strength)} structural change</p><p className="mt-1 text-xs leading-5 text-zinc-400">Built-up interpretation: {titleCase(region.semantic_support_level)} · {titleCase(region.directional_state)}</p><p className="mt-2 text-[11px] text-zinc-500">{region.pixel_area.toLocaleString()} px · {region.relative_area_percent.toFixed(3)}% of analysis grid</p></article>)}</div></div>}
      <p className="mt-4 border-t border-white/[.07] pt-4 text-xs leading-5 text-zinc-500">{summary.caveats[0] ?? "The interpretation remains subordinate to the measured evidence."}</p>
    </article>
  </section>;
}

function LimitationsPanel({ groups, compact }: { groups: Record<LimitationGroup, string[]>; compact: boolean }) {
  return <div className="result-limitations-grid">{(Object.keys(groups) as LimitationGroup[]).map(group => <section key={group}><h3>{group}</h3>{groups[group].length ? <ul>{groups[group].slice(0, compact ? 4 : undefined).map(item => <li key={item}>{item}</li>)}</ul> : <p>No {group.toLowerCase()} limitation was published.</p>}</section>)}</div>;
}

function GroupedTimeline({ result }: { result: AgentResponse }) {
  const order: TimelineGroup[] = ["Input", "Validation", "Model Inference", "Evidence Fusion", "Response"];
  const groups = order.map(name => ({ name, steps: result.execution.steps.map((step, index) => ({ step, index })).filter(({ step }) => timelineGroup(step.tool) === name) }));
  return <div className="result-timeline-groups">{groups.map(({ name, steps }, groupIndex) => <details key={name} open={groupIndex < 2 || steps.some(({ step }) => step.status === "failed")}><summary><span>{name}</span><small>{steps.length} stage{steps.length === 1 ? "" : "s"} · {steps.reduce((total, { step }) => total + step.duration_ms, 0).toLocaleString()} ms</small></summary>{steps.length ? <ol className="result-timeline">{steps.map(({ step, index }) => <li key={`${step.tool}-${index}`}><span className={cn(step.status === "success" ? "result-timeline-success" : step.status === "failed" ? "result-timeline-error" : "result-timeline-warning")}>{step.status === "success" ? <Check size={15}/> : index + 1}</span><div><strong>{timelineLabel(step.tool)}</strong><small>{step.duration_ms.toLocaleString()} ms · {titleCase(step.status)}</small>{typeof step.parameters.reason === "string" && <small>{step.parameters.reason}</small>}</div></li>)}</ol> : <p className="result-timeline-empty">No stages were published in this group.</p>}</details>)}</div>;
}
...
export function AssistantResultExperience({ result, changeResult, query, onRerun, rerunning, children }: Props) {
  ...
  const limitations = useMemo(() => groupedLimitations(result, changeResult), [changeResult, result]);
  const successful = result.status === "success" || result.status === "partial";
  const specialists = presentedSpecialists(result, changeResult);
  ...
  const semanticSummary = result.semantic_change_summary ?? changeResult?.semantic_change_summary;
  const reasons = Array.from(new Set([
    ...(semanticSummary?.supporting_facts ?? []),
  ...
  const translatedProduct = authoritativeTranslatedEvidence(result);
  const legacyTranslationSuppressed = Boolean(result.sar_translated_optical_analysis?.generated_preview_url && !translatedProduct);
  ...
  return <section className="result-experience mt-10" aria-labelledby={`result-title-${result.request_id}`}>
    ...
    <article className="result-hero mt-4">
      ...
      <dl className="result-hero-facts"><div><dt>Workflow</dt><dd>{workflowLabel(result.execution.input_mode)}</dd></div><div><dt>Status</dt><dd>{titleCase(result.result_status || result.status)}</dd></div><div><dt>Runtime</dt><dd>{result.execution.duration_ms.toLocaleString()} ms</dd></div><div><dt>{result.sar_water_analysis ? "Heuristic reliability" : "Confidence"}</dt><dd>{result.sar_water_analysis?.heuristic_reliability != null ? `${(result.sar_water_analysis.heuristic_reliability * 100).toFixed(1)}% · not model confidence` : `${titleCase(result.confidence.level)}${result.confidence.score !== null ? ` · ${(result.confidence.score * 100).toFixed(1)}%` : ""}`}</dd></div><div><dt>Primary Specialist</dt><dd>{specialists.primary}</dd></div><div><dt>Supporting Specialists</dt><dd>{specialists.supporting.join(" · ") || "None published"}</dd></div></dl>
      <ConfidenceDisclosure reason={result.confidence.reason}/>
    ...
    {semanticSummary && <SemanticChangePanel summary={semanticSummary}/>}

    {translatedProduct && <aside className="mt-4 rounded-2xl border border-amber-300/25 bg-amber-300/[.06] p-5 text-sm leading-6 text-amber-50" aria-label="Generated optical-like image disclosure"><strong className="block text-amber-200">Generated representation — supporting evidence only</strong><p className="mt-1">{result.sar_translated_optical_analysis?.disclosure}</p>{result.sar_translated_optical_analysis?.semantic_comparison_state === "SUCCEEDED" && <p className="mt-2 text-xs text-amber-100/70">Agreement: {titleCase(result.sar_translated_optical_analysis.agreement)} · Model: {result.sar_translated_optical_analysis.model ?? "Unavailable"}</p>}</aside>}
    {legacyTranslationSuppressed && <p className="mt-4 rounded-xl border border-amber-300/20 bg-amber-300/[.04] p-3 text-xs text-amber-100" role="status">Legacy generated evidence provenance unavailable. The generated layer was suppressed.</p>}

    {sve && <div className="mt-4"><RemoteSensingAdaptationPanel result={sve}/></div>}

    <EvidenceSources result={result} changeResult={changeResult}/>
    ...
    {engine && <section className="result-section" aria-labelledby={`engine-${result.request_id}`}>
      ...
      <div className="grid gap-4 lg:grid-cols-[.85fr_1.15fr]">
        <article className="result-explain-card"><h3><Layers3 size={18}/>{engine.fallback_used ? "Deterministic Fallback" : `${changeEngineName(engine, ttp)} Change Analysis`}</h3><dl className="mt-4 grid gap-3 text-sm"><div><dt className="text-zinc-500">Primary specialist</dt><dd className="mt-1 text-zinc-200">{specialists.primary}</dd></div><div><dt className="text-zinc-500">Supporting evidence</dt><dd className="mt-1 text-zinc-200">{specialists.supporting.join(" · ") || "None published"}</dd></div><div><dt className="text-zinc-500">Automatic fallback</dt><dd className="mt-1 text-zinc-200">Enabled{engine.fallback_used ? ` · used (${specialists.fallbackReason})` : " · not used"}</dd></div></dl></article>
        <article className="result-explain-card"><h3><ShieldCheck size={18}/>{changeEngineName(engine, ttp)} provenance</h3>{ttp?.status === "success" ? <dl className="mt-4 grid gap-3 text-sm sm:grid-cols-2"><div><dt className="text-zinc-500">Model</dt><dd className="mt-1 text-zinc-200">{ttp.model}</dd></div><div><dt className="text-zinc-500">Architecture</dt><dd className="mt-1 text-zinc-200">{ttp.architecture}</dd></div><div><dt className="text-zinc-500">Training dataset</dt><dd className="mt-1 text-zinc-200">{ttp.training_dataset}</dd></div><div><dt className="text-zinc-500">Checkpoint</dt><dd className="mt-1 text-zinc-200">{ttp.checkpoint}</dd></div><div><dt className="text-zinc-500">Runtime device</dt><dd className="mt-1 text-zinc-200">{ttp.device ?? "Unavailable"}</dd></div><div><dt className="text-zinc-500">Model reuse</dt><dd className="mt-1 text-zinc-200">{ttp.reused_model ? "Warm model reused" : "Loaded for first inference"}</dd></div></dl> : <p className="mt-4 text-sm leading-6 text-amber-100">{changeEngineName(engine, ttp)} evidence is unavailable for this result. The deterministic fallback reason is disclosed above.</p>}{consistency && <p className="mt-4 text-xs leading-5 text-zinc-400"><strong className="text-zinc-200">{consistency.label}.</strong> {consistency.disclosure}</p>}</article>
      </div>
      <ChangeScientificSummary result={result} change={changeResult}/>
    </section>}
    ...
    <section className="result-section" aria-labelledby={`why-${result.request_id}`}><div className="result-section-heading"><div><p className="result-section-number">04</p><h2 id={`why-${result.request_id}`}>Why This Answer?</h2></div><p>Published evidence, routing rationale, confidence, and limitations.</p></div><article className="result-explain-card"><h3><ShieldCheck size={18}/>Backend-published rationale</h3>{reasons.length ? <ul>{reasons.map(reason => <li key={reason}><CheckCircle2 size={16}/><span>{reason}</span></li>)}</ul> : <p className="mt-4 text-sm text-zinc-500">No explanatory evidence text was published. The browser does not invent a rationale.</p>}</article><div className="mt-4"><LimitationsPanel groups={limitations} compact={view === "judge"}/></div>
      <article className="result-decision-card"><div><span>Query</span><strong>{query}</strong></div><i>→</i><div><span>Task classification</span><strong>{titleCase(result.task)}</strong></div><i>→</i><div><span>Selected specialist</span><strong>{specialists.primary}</strong></div><i>→</i><div><span>Reason</span><strong>{specialists.fallbackReason ?? result.execution.selection_reason}</strong></div><i>→</i><div><span>Execution</span><strong>{titleCase(result.status)}</strong></div></article>
    </section>

    <section className="result-section" aria-labelledby={`timeline-${result.request_id}`}><div className="result-section-heading"><div><p className="result-section-number">05</p><h2 id={`timeline-${result.request_id}`}>Execution Timeline</h2></div><p>Every observable backend stage, grouped for review without hiding the underlying record.</p></div><GroupedTimeline result={result}/></section>
