...
const semanticSummary = {
  ...
  expanded_answer: "The learned evidence identifies localized change in the northeast, while most of the remaining scene is comparatively stable.",
  query_intent: "change_summary", overall_change_level: "localized", dominant_location: "northeast",
  likely_change_type: "unknown_change", evidence_strength: "moderate", stable_area_summary: "Most of the scene remains stable.",
  changed_regions: ["northeast"], stable_regions: ["central", "western"], likely_transitions: [],
  supporting_facts: ["Localized learned change", "Northeast concentration"],
  caveats: ["Semantic type is not established."], visual_structural_disagreement: false,
  generated_by: "local_semantic_interpreter" as const,
};

function result(cached = false): AgentResponse {
  return {
    request_id: "request-change", task: "change_description",
    answer: semanticSummary.expanded_answer,
    semantic_change_summary: semanticSummary,
...
describe("PresentationChangeScene", () => {
  ...
  it("renders the real result contract, statistics, controlled answer, and all evidence products", async () => {
    ...
    expect(container.querySelector('img[alt^="Difference Heatmap"]')).toBeTruthy();
