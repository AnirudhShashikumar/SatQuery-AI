...
class GrounderUnitTests(unittest.TestCase):
    ...
    def test_health_does_not_force_load(self):
        ...
        self.assertFalse(grounder.health().smoke_verified)
        self.assertEqual(FakeModel.load_count, 0)

    def test_explicit_complete_local_snapshot_is_resolved_offline(self):
        with tempfile.TemporaryDirectory() as directory:
            for name in ("config.json", "preprocessor_config.json", "tokenizer_config.json", "model.safetensors", "tokenizer.json"):
                open(os.path.join(directory, name), "wb").close()
            with patch.dict(os.environ, {"SATQUERY_GROUNDER_LOCAL_MODEL": directory}, clear=False):
                resolved = RemoteSensingGrounder()._verified_local_snapshot()
            self.assertEqual(resolved, __import__("pathlib").Path(directory).resolve())

    def test_invalid_local_path_and_missing_artifacts_are_actionable(self):
        with patch.dict(os.environ, {"SATQUERY_GROUNDER_LOCAL_MODEL": "/definitely/not/a/grounder"}, clear=False):
            with self.assertRaisesRegex(GrounderError, "path is invalid"):
                RemoteSensingGrounder()._verified_local_snapshot()
        with tempfile.TemporaryDirectory() as directory:
            with patch.dict(os.environ, {"SATQUERY_GROUNDER_LOCAL_MODEL": directory}, clear=False):
                with self.assertRaisesRegex(GrounderError, "model files missing"):
                    RemoteSensingGrounder()._verified_local_snapshot()
    ...
    def test_load_failure_is_sanitized_and_sticky(self):
        ...
        with patch.object(grounder, "_load_components", side_effect=RuntimeError("secret /private/model/path")):
            with self.assertRaisesRegex(GrounderError, "initialization failed"):
                grounder.load()
        health = grounder.health()
        self.assertEqual(health.status, "failed")
        self.assertEqual(health.error, "Grounding DINO initialization failed.")
    ...
    def test_lazy_load_reuse_detection_preview_provenance_and_confidence(self):
        ...
        self.assertTrue(first.model.remote_sensing_adapted)
        self.assertEqual(first.model.adaptation_dataset, "VRSBench")
        self.assertIn("Grounding Specialist v1.1", first.model.base_architecture)
    ...
    def test_real_cached_checkpoint_contract(self):
        ...
        self.assertTrue(result.model.remote_sensing_adapted)
...
class GroundingApiTests(unittest.TestCase):
    ...
    def test_health_exposes_grounder_without_forcing_load(self):
        ...
        self.assertIn("grounding_specialist_v1_1", health["specialists"])
        self.assertEqual(grounder.health().status, "unloaded")
