...
class SatQueryAgentTests(unittest.TestCase):
    ...
    def test_grounding_route(self):
        ...
        # A verified local grounding bundle makes the tool available; hosts
        # without it still disclose the unavailable specialist as skipped.
        grounding_status = body["execution"]["steps"][-1]["status"]
        self.assertIn(grounding_status, {"not_implemented", "skipped"})
        warning_text = " ".join(body["warnings"])
        self.assertIn("model-produced evidence" if grounding_status == "not_implemented" else "multipart", warning_text)
    ...
    def test_urban_expansion_location_routes_to_change_vqa(self):
        body = self.query(
            "Where has urban expansion occurred?",
            input_mode="bi_temporal",
            secondary_modality="optical",
            has_secondary_image=True,
        )
        self.assertEqual(body["task"], "change_vqa")
        self.assertEqual(body["execution"]["selected_tools"][-1], "bitemporal_change_analyzer")
    ...
    def test_tool_registry_response(self):
        ...
        self.assertEqual(tools["changerex_change_detector"]["adaptation_dataset"], "LEVIR-CD")
