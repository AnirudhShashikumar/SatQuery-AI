...
class TTPClientTests(unittest.TestCase):
    ...
    def test_health_payload_exposes_complete_lifecycle_without_paths(self):
        ready = {**HEALTH, "model_load_count": 1, "model_reuse_count": 7, "inference_count": 8}
        session = FakeSession(response(200, self.payload()))
        session.get = lambda url, **kwargs: response(200, ready)  # type: ignore[method-assign]
        client = TTPServiceClient(session)
        with patch.dict(os.environ, {"TTP_ENABLED": "true", "TTP_SERVICE_URL": "http://service.internal"}):
        ...
        self.assertEqual(health, {
            ...
            "reuse_count": 7, "inference_count": 8, "errors": [],
        })
