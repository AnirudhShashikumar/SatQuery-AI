...
def compliance_summary() -> ComplianceResponse:
    ...
    evidence = {
        "Single-image VQA": ("VERIFIED", "RSVQA-LR official test · 10,004 questions · 71.05% exact · 74.31% overflow-aware", "verified_test"),
        "Text-guided grounding": ("VERIFIED", "VRSBench validation · 16,146 samples · mIoU 0.2348 · Acc@0.50 25.05%", "verified_validation"),
        "Learned optical change evidence": ("PARTIALLY VERIFIED", "ChangerEx parity and operational smoke; official LEVIR-CD quality benchmark pending", "operational_only"),
        "Captioning": ("PARTIALLY VERIFIED", "Reference-caption benchmark pending", "unavailable"),
        "Optical–SAR joint analysis": ("IMPLEMENTED", "Controlled labelled comparison pending", "unavailable"),
        "Change VQA": ("IMPLEMENTED", "CDVQA evaluator ready; dataset run pending", "unavailable"),
    }
    requirements = [
        item.model_copy(update={
            "readiness_status": evidence.get(item.requirement, ("IMPLEMENTED", None, "operational_only"))[0],
            "specialists": [item.tool_or_model],
            "benchmark_evidence": evidence.get(item.requirement, (None, "No task-specific public benchmark applies.", None))[1],
            "evidence_status": evidence.get(item.requirement, (None, None, "operational_only"))[2],
        })
