...
from .bitemporal_semantics import classify_change_intent
...
def classify_change_question(question: str) -> QuestionIntent:
    ...
    if any(term in normalized for term in ("what caused", "why did", "who caused", "reason for")):
        return QuestionIntent(QuestionCategory.UNSUPPORTED)
    semantic_intent = classify_change_intent(question)
    semantic_categories = {
        "largest_change": QuestionCategory.LARGEST_CHANGE,
        "change_location": QuestionCategory.CHANGE_LOCATION,
        "amount_of_change": QuestionCategory.CHANGE_PERCENTAGE,
        "built_up_change": QuestionCategory.BUILT_UP_CHANGE,
        "vegetation_change": QuestionCategory.VEGETATION_CHANGE,
        "water_change": QuestionCategory.WATER_CHANGE,
        "infrastructure_change": QuestionCategory.INFRASTRUCTURE_CHANGE,
        "no_change_check": QuestionCategory.CHANGE_MAGNITUDE,
        "general_comparison": QuestionCategory.CHANGE_SUMMARY,
        "change_summary": QuestionCategory.CHANGE_SUMMARY,
    }
    if semantic_intent in semantic_categories:
        return QuestionIntent(semantic_categories[semantic_intent], semantic_intent)
...
def _direction(box: Any, width: int, height: int) -> str:
    ...
    return f"the {vertical}-{horizontal} of the image"
...
def answer_change_question(question: str, change: ChangeAnalysisResponse) -> ControlledAnswer:
    ...
    learned_name = "ChangerEx" if change.change_engine and change.change_engine.primary_tool == "changerex_change_detector" else "TTP"
    primary_label = f"{learned_name} learned mask" if hybrid else "deterministic change mask"
    confidence_reason = (
    ...
    if statistics is None:
    ...
    elif change.semantic_change_summary is not None and change.semantic_change_summary.query_intent != "unknown" and supported:
        semantic = change.semantic_change_summary
        used = {
            "percentage_changed": statistics.percentage_changed,
            "number_of_regions": statistics.number_of_regions,
            "largest_connected_region": statistics.largest_connected_region,
            "semantic_query_intent": semantic.query_intent,
            "overall_change_level": semantic.overall_change_level,
            "dominant_location": semantic.dominant_location,
            "likely_change_type": semantic.likely_change_type,
            "semantic_evidence_strength": semantic.evidence_strength,
        }
        answer = semantic.expanded_answer if semantic.query_intent in {"change_summary", "general_comparison"} else semantic.short_answer
        confidence = Confidence(
            level=ConfidenceLevel.LOW if semantic.evidence_strength == "limited" else ConfidenceLevel.MODERATE,
            score=None,
            reason=(
                f"Local evidence-gated interpretation with {semantic.evidence_strength} evidence. "
                "This is not a calibrated probability; semantic scene evidence is advisory."
            ),
        )
        ...
        if hybrid and change.deterministic_statistics and change.mask_comparison:
            answer = (
                f"The {learned_name} learned detector identified {statistics.percentage_changed:.1f}% changed pixels. "
    ...
    semantic_enabled = bool(change.semantic_change_summary is not None and change.semantic_change_summary.query_intent != "unknown" and supported)
    method_name = "local evidence-gated bi-temporal semantic interpretation" if semantic_enabled else f"controlled answers over {learned_name} primary and deterministic supporting change statistics" if hybrid else "controlled answers over deterministic change statistics"
    assumptions = (["Structured semantic facts are derived before prose; the binary mask remains authoritative for location and magnitude."] if semantic_enabled else []) + ([f"The {learned_name} learned mask is primary and deterministic evidence is supporting; agreement is not ground-truth accuracy."] if hybrid else ["The protected deterministic change mask is the evidence source."])
    limitations = ([
        "Semantic transitions require compatible supporting scene evidence and are phrased cautiously.",
        "SVE similarities are scene-level, uncalibrated, and not spatial segmentation or physical percentages.",
        "The system does not establish real-world cause or count newly constructed objects.",
    ] if semantic_enabled else CHANGE_LIMITATIONS) + ([f"{learned_name} was trained on LEVIR-CD building-change imagery and may generalize weakly outside that domain.", f"{learned_name} output is not ground truth."] if hybrid else [])
    ...
    return ControlledAnswer(answer, ControlledVQAResult(
        ...
        answer_source="local structured semantic interpretation over published evidence" if semantic_enabled else f"hybrid {learned_name}-primary change statistics" if hybrid else "deterministic bi-temporal change statistics", statistics_used=used,
...
CROSS_LIMITATIONS = [
    ...
    "Quantitative joint answering requires exact pixel and geospatial alignment; qualitative source comparison does not.",
]


def cross_modal_requires_quantitative(question: str) -> bool:
    return bool(re.search(r"percent|%|overlap|intersection|\biou\b|\barea\b|hectare|square|coordinate|latitude|longitude|pixel|how many|exactly", _normalize(question)))
...
def classify_cross_modal_question(question: str) -> QuestionIntent:
    ...
    if any(term in normalized for term in UNSUPPORTED_CAUSAL):
        return QuestionIntent(QuestionCategory.UNSUPPORTED)
    if any(term in normalized for term in ("limitation", "caveat", "cannot conclude")):
        return QuestionIntent(QuestionCategory.CROSS_MODAL_AGREEMENT, "limitations")
    if any(term in normalized for term in ("confidence", "how certain", "how reliable")):
        return QuestionIntent(QuestionCategory.CROSS_MODAL_AGREEMENT, "confidence")
    has_water = any(term in normalized for term in ("water", "river", "lake", "reservoir"))
    has_structure = any(term in normalized for term in ("structural", "built up", "built-up", "building", "urban"))
    if has_water and has_structure:
        return QuestionIntent(QuestionCategory.CROSS_MODAL_AGREEMENT, "water_and_structure")
    if any(term in normalized for term in ("sar reveal", "radar reveal", "sar only", "radar only", "sar show that optical")):
        return QuestionIntent(QuestionCategory.CROSS_MODAL_DISAGREEMENT, "sar_complementary")
    ...
    if cross_modal_requires_quantitative(question) and not (has_water or has_structure):
        return QuestionIntent(QuestionCategory.CROSS_MODAL_AGREEMENT, "quantitative")
    if has_water:
        return QuestionIntent(QuestionCategory.CROSS_MODAL_WATER, "water")
    if has_structure or "structurally complex" in normalized:
...
def summarize_image_sectors(locations: list[str]) -> str:
    """Compress discrete sector support, never infer pixel coverage or orientation."""
    locations = sorted(set(locations))
    if len(locations) == 9:
        return "all nine image-relative sectors"
    if len(locations) > 4:
        return f"{len(locations)} of the nine image-relative sectors"
    for row in ("upper", "middle", "lower"):
        if set(locations) == {f"{row}-{column}" for column in ("left", "center", "right")}:
            return f"the {row} row of image-relative sectors"
    return "the " + ", ".join(locations) + (" image-relative sector" if len(locations) == 1 else " image-relative sectors")
...
def answer_cross_modal_question(question: str, result: CrossModalResult) -> ControlledAnswer:
    ...
    if intent.target in {"confidence", "limitations"}:
        if intent.target == "confidence":
            answer = f"The published evidence has {result.confidence.level.value} confidence. {result.confidence.reason} This is not a calibrated probability."
        else:
            limits = list(dict.fromkeys(result.limitations + result.method.limitations + result.warnings))
            answer = (("This pair supports qualitative, image-relative comparison only. " if result.analysis_level == "qualitative" else "") + " ".join(limits[:3])) or "No workflow-specific limitations were published; this does not establish unrestricted validity."
        confidence = result.confidence
        used = {"analysis_level": result.analysis_level, "quantitative_metrics_available": result.quantitative_metrics_available, "review_intent": intent.target}
        references = [item.reference for item in result.evidence_products if item.reference]
    elif result.analysis_level == "qualitative" and supported and not cross_modal_requires_quantitative(question):
        selected = result.agreements
        if intent.category == QuestionCategory.CROSS_MODAL_WATER:
            selected = [item for item in selected if item.category == "water"]
        elif intent.category == QuestionCategory.CROSS_MODAL_STRUCTURE:
            selected = [item for item in selected if item.category == "structure"]
        statements = []
        for category in dict.fromkeys(item.category for item in selected):
            findings = [item for item in selected if item.category == category]
            locations = summarize_image_sectors([item.relative_location for item in findings])
            name = "water-like" if category == "water" else "structural"
            statements.append(f"Both observations support {name} candidates in {locations}.")
        if intent.category == QuestionCategory.CROSS_MODAL_DISAGREEMENT:
            statements = result.complementary_findings if intent.target == "sar_complementary" else result.disagreements
        if not statements:
            statements = ["The independent evidence rules did not establish the requested agreement or complementary finding; this does not prove absence."]
        answer = " ".join(statements[:2]) + " This qualitative comparison uses optical appearance and native SAR backscatter/texture; candidates are not confirmed semantic classes. Pixel alignment is assumed from the supplied pairing; geospatial co-registration cannot be independently verified. Locations refer to image sectors, not geographic directions."
        if intent.target == "water_and_structure":
            for category in ("water", "structure"):
                if not any(item.category == category for item in selected):
                    answer += f" Joint {category} support was not established by these candidate rules."
        confidence = result.confidence
        used = {"analysis_level": "qualitative", "evidence_sources": ["optical", "native_sar", "fused"], "quantitative_metrics_available": False}
        references = [item.reference for item in result.evidence_products if item.reference]
        ...
        sar_only = [
            ...
            if region.type == "disagreement" and region.support.sar and not region.support.optical
        ]
        used = {"disagreement_percent": value, "sar_only_region_count": len(sar_only), "evidence_sources": ["optical", "native_sar", "fused"]}
        if value is None:
            answer = "No candidate pixels were available for a cross-modal comparison."
        elif intent.target == "sar_complementary":
            answer = (
                f"Native SAR contributes relative backscatter and texture evidence in {len(sar_only)} region(s) not supported by the optical candidate rules. "
                f"Overall modality disagreement is approximately {value:.1f}% of evaluated candidate-evidence pixels; these regions are not confirmed semantic classes."
            )
        else:
            answer = f"Optical and native SAR candidate evidence disagrees over approximately {value:.1f}% of evaluated candidate-evidence pixels."
        confidence = result.confidence
        ...
        used = {"agreement_percent": value, "joint_region_count": len(joint), "evidence_sources": ["optical", "native_sar", "fused"]}
        valid = statistics.valid_pixel_percent or 0.0
        used["valid_pixel_percent"] = valid
        if value is None:
            answer = "No candidate pixels were available for an agreement calculation."
        elif intent.target == "water_and_structure":
            answer = (
                f"Optical and native SAR evidence jointly supports water-like conditions over approximately {statistics.water_likelihood_percent or 0.0:.1f}% "
                f"and structural-likelihood conditions over approximately {statistics.built_up_likelihood_percent or 0.0:.1f}% of valid pixels. "
                f"They positively agree over {value:.1f}% of evaluated candidate-evidence pixels across {len(joint)} joint region(s)."
            )
        else:
            answer = f"The modalities positively agree over approximately {value:.1f}% of evaluated candidate-evidence pixels across {len(joint)} joint evidence region(s); {valid:.1f}% of source pixels were valid pixels."
        confidence = result.confidence
    ...
    method = _method("controlled answers over deterministic optical-SAR fusion", ["Answers use published native-source findings or protected exact-grid statistics."], CROSS_LIMITATIONS)
    return ControlledAnswer(answer, ControlledVQAResult(
        original_question=question, question_category=intent.category, target_concept=intent.target,
        answer_source="deterministic optical-SAR qualitative native evidence" if result.analysis_level == "qualitative" else "deterministic optical-SAR evidence statistics", statistics_used=used,
