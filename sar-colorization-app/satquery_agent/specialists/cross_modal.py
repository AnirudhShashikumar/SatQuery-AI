...
METHOD_ASSUMPTIONS = [
    ...
    "Optical roles are validated against detected content; the first three prepared bands are treated as red, green, and blue when explicit names are unavailable.",
...
def _prepare_optical(array: np.ndarray, metadata: ImageMetadata) -> PreparedOptical:
    ...
    rgb_like = raw.shape[2] >= 3 and metadata.effective_modality not in {ImageModality.OPTICAL_GRAYSCALE, ImageModality.PANCHROMATIC}
    if raw.shape[2] == 1:
        raw = np.repeat(raw, 3, axis=2)
    elif raw.shape[2] == 2:
        raw = np.dstack((raw, np.mean(raw, axis=2)))
    normalized, spans, limits = _normalize_channels(raw, valid)
    bands = (metadata.selected_visual_bands[:3] or metadata.color_interpretation[:3] or [f"band_{index + 1}" for index in range(min(3, metadata.band_count))])
    if len(bands) < 3:
...
def _qualitative_eligible(compatibility, optical_metadata, sar_metadata):
    return (
        compatibility.compatible
        and compatibility.alignment_level == AlignmentLevel.VISUAL_ONLY
        and compatibility.same_dimensions
        and not compatibility.resampling_required
        and all(metadata.format in {ImageFormat.PNG, ImageFormat.JPEG}
                and metadata.representation == RepresentationType.DISPLAY_PREVIEW
                and not metadata.crs and not metadata.transform and not metadata.is_georeferenced
                for metadata in (optical_metadata, sar_metadata))
    )


def _sector_support(mask: np.ndarray) -> set[str]:
    # Independent coarse summaries, never a pixel intersection or overlap metric.
    locations = set()
    for row, rows in enumerate(np.array_split(mask, 3, axis=0)):
        for column, sector in enumerate(np.array_split(rows, 3, axis=1)):
            if sector.size and np.count_nonzero(sector) >= max(6, int(np.ceil(sector.size * 0.02))):
                locations.add(f"{('upper', 'middle', 'lower')[row]}-{('left', 'center', 'right')[column]}")
    return locations


def _qualitative_result(optical, sar, optical_metadata, sar_metadata, compatibility, durations, started_total):
    started = time.perf_counter()
    optical_water, optical_structure, vegetation, _ = _optical_evidence(optical)
    if not optical.dynamic_range_ok:
        optical_water[:] = False
        optical_structure[:] = False
        vegetation[:] = False
    durations["optical_evidence_extraction"] = _duration(started)
    started = time.perf_counter()
    sar_water, sar_structure, _ = _sar_evidence(sar)
    durations["sar_evidence_extraction"] = _duration(started)
    started = time.perf_counter()
    optical_findings, sar_findings, agreements, differences, facts = [], [], [], [], []
    categories = [
        ("water", optical_water, sar_water, "dark/blue smooth optical appearance consistent with water", "low-backscatter evidence consistent with water; radar shadow and smooth terrain remain alternatives"),
        ("structure", optical_structure, sar_structure, "visible edges and texture consistent with structural terrain", "structural/high-return evidence consistent with built-up terrain; bright terrain and vegetation remain alternatives"),
    ]
    for category, optical_mask, sar_mask, optical_support, sar_support in categories:
        optical_locations, sar_locations = _sector_support(optical_mask), _sector_support(sar_mask)
        optical_text = f"Optical {category} candidates: {', '.join(sorted(optical_locations)) or 'none under the extraction rules'}. Support: {optical_support}."
        sar_text = f"Native SAR {category} candidates: {', '.join(sorted(sar_locations)) or 'none under the extraction rules'}. Support: {sar_support}."
        optical_findings.append(optical_text)
        sar_findings.append(sar_text)
        facts.extend([CrossModalEvidenceFact(source="optical", kind=category, statement=optical_text), CrossModalEvidenceFact(source="native_sar", kind=category, statement=sar_text)])
        for location in sorted(optical_locations & sar_locations):
            agreements.append(CrossModalAgreement(category=category, relative_location=location, optical_support=optical_support, sar_support=sar_support))
        for location in sorted(sar_locations - optical_locations):
            differences.append(f"Native SAR adds {category} candidate evidence in the {location} image-relative sector without optical candidate support; this is not a confirmed semantic class or verified spatial difference.")
        for location in sorted(optical_locations - sar_locations):
            differences.append(f"Optical {category} candidates in the {location} image-relative sector lack corresponding native SAR candidate support.")
    joint = [f"Qualitative {item.category} agreement in the {item.relative_location} image-relative sector: optical {item.optical_support}; native SAR {item.sar_support}." for item in agreements]
    if not joint:
        joint = ["No coarse image-relative agreement survived the independent candidate rules; absence of candidate support is not proof of absence."]
    facts.extend(CrossModalEvidenceFact(source="fused", kind="qualitative_agreement", statement=value) for value in joint)
    durations["joint_evidence_fusion"] = _duration(started)
    started = time.perf_counter()
    saved = []
    try:
        for image in (Image.fromarray(np.round(optical.rgb * 255).astype(np.uint8)), Image.fromarray(np.round(sar.intensity * 255).astype(np.uint8)), _evidence_composite(optical_water.shape, optical_water, optical_structure, vegetation), _evidence_composite(sar_water.shape, sar_water, sar_structure)):
            try:
                saved.append(save_preview(image))
            finally:
                image.close()
    ...
    limitations = _unique(compatibility.warnings + METHOD_LIMITATIONS + [
        "Qualitative analysis only. Locations use image top/bottom/left/right, not geographic north/south/east/west.",
        "Sector co-occurrence is not verified pixel agreement. No intersection masks, joint percentages, coordinates or metric areas were computed.",
        "SAR preview intensity is relative and uncalibrated. Confidence is not a calibrated probability.",
    ])
    low_information = not optical.dynamic_range_ok or not sar.dynamic_range_ok or not agreements
    return CrossModalResult(
        status=CrossModalStatus.SUCCESS, analysis_level="qualitative", quantitative_metrics_available=False,
        summary=CrossModalSummary(optical_observations=optical_findings, sar_observations=sar_findings, joint_observations=joint),
        agreements=agreements, disagreements=differences,
        complementary_findings=[text for text in differences if text.startswith("Native SAR")], limitations=limitations,
        previews=CrossModalPreviewUrls(optical=saved[0], sar=saved[1], optical_evidence=saved[2], sar_evidence=saved[3]),
        confidence=Confidence(level=ConfidenceLevel.LOW if low_information else ConfidenceLevel.MODERATE, score=None,
            reason="Both native sources were analyzed independently with conservative candidate rules; registration and semantic certainty remain unverified. This is not a calibrated probability."),
        method=_method(), optical_preparation=optical.preparation, sar_preparation=sar.preparation,
        warnings=_unique(optical_metadata.warnings + sar_metadata.warnings + limitations),
        runtime_ms=_duration(started_total), stage_durations_ms=durations, evidence_facts=facts,
    )


def publish_cross_modal_products(result, optical_metadata, sar_metadata):
    products = []
    for name, url in result.previews.model_dump().items():
        if not url:
            continue
        source = optical_metadata if name in {"optical", "optical_evidence", "vegetation_support"} else sar_metadata if name in {"sar", "sar_evidence"} else None
        evidence_type = "source" if name in {"optical", "sar"} else "native_evidence" if source else "joint_evidence"
        # Source products point to the original observation preview, not normalized analysis output.
        if evidence_type == "source":
            url = source.preview_url or url
        products.append(EvidenceItem(type="preview", label=f"{'SAR' if name == 'sar' else 'Optical'} source" if evidence_type == "source" else name.replace("_", " ").title(),
            reference=url, evidence_id=f"{optical_metadata.file_id}:{sar_metadata.file_id}:{name}",
            source_observation_id=source.file_id if source else None,
            source_observation_ids=[source.file_id] if source else [optical_metadata.file_id, sar_metadata.file_id],
            source_role=source.observation_role if source else None,
            source_modality=source.auto_detected_modality if source else None, evidence_type=evidence_type,
            description="Native source evidence; candidate support is not semantic ground truth."))
    return result.model_copy(update={"evidence_products": products})


class CrossModalOpticalSarAnalyzer:
    """Native-source qualitative analysis with a separate exact-grid quantitative gate."""
    ...
    def analyze(
        ...
        qualitative = _qualitative_eligible(compatibility, optical_metadata, sar_metadata)
        if not compatibility.compatible or (compatibility.alignment_level != AlignmentLevel.EXACT and not qualitative):
        ...
        if qualitative:
            return _qualitative_result(optical, sar, optical_metadata, sar_metadata, compatibility, durations, started_total)
        ...
        water_regions = [region.region_id for region in regions if region.type == "water_likelihood"]
        structural_regions = [region.region_id for region in regions if region.type == "built_up_likelihood"]
        disagreement_regions = [region.region_id for region in regions if region.type == "disagreement"]
        evidence_facts = [
            CrossModalEvidenceFact(
                source="optical",
                kind="visible_spectrum",
                statement="The optical branch contributed visible-spectrum water, vegetation, and structural candidate evidence where RGB bands were established.",
            ),
            CrossModalEvidenceFact(
                source="native_sar",
                kind="relative_backscatter_and_texture",
                statement="The native SAR branch contributed relative low-intensity, strong-response, and texture evidence without using translated imagery.",
            ),
            CrossModalEvidenceFact(
                source="fused",
                kind="water_likelihood",
                statement=water_text,
                supporting_region_ids=water_regions,
            ),
            CrossModalEvidenceFact(
                source="fused",
                kind="structural_likelihood",
                statement=structural_text,
                supporting_region_ids=structural_regions,
            ),
        ]
        if disagreement_regions:
            evidence_facts.append(CrossModalEvidenceFact(
                source="fused",
                kind="disagreement",
                statement=agreement_text,
                supporting_region_ids=disagreement_regions,
            ))
        return CrossModalResult(
            status=status,
            analysis_level="pixel_verified",
            quantitative_metrics_available=True,
            summary=CrossModalSummary(
