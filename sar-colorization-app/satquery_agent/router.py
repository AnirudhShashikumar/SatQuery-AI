...
from .specialists.vqa import classify_cross_modal_question, get_vqa
...
TASK_RULES: Sequence[Tuple[TaskType, Sequence[str], str, str]] = (
    ...
    (TaskType.CHANGE_VQA, ("has built-up increased", "has vegetation decreased", "did water coverage change", "urban expansion", "built-up expansion", "built up expansion", "increased, decreased, or unchanged", "how much of the image changed", "how much of image changed", "where did the largest change", "where is the largest changed", "how many major changed regions", "mostly unchanged", "amount of change", "small, moderate, or large", "cause of change", "caused the change", "change caused"), "bitemporal_change_analyzer", "The query asks a controlled question about measured change between dates."),
...
def detect_task(query: str) -> Tuple[TaskType, List[str], str]:
    normalized = normalize_query(query)
    directional_change_question = (
        any(subject in normalized for subject in ("built-up", "built up", "vegetation", "water body", "water coverage"))
        and any(direction in normalized for direction in ("increase", "increased", "decrease", "decreased", "remain unchanged", "remained unchanged"))
    )
    if directional_change_question:
        return (
            TaskType.CHANGE_VQA,
            ["input_validator", "bitemporal_change_analyzer"],
            "The query asks whether an evidence-supported scene characteristic changed between observations.",
        )
...
def route_query(request: AgentQueryRequest) -> RoutingPlan:
    ...
    cross_modal_intent = classify_cross_modal_question(request.query)
    if request.input_mode == InputMode.SINGLE and re.fullmatch(r"(?:is this (?:image |observation )?(?:sar|radar|optical)(?: imagery)?|what (?:is the )?(?:image |observation )?modality(?: is this)?)\??", normalize_query(request.query)):
        task = TaskType.VQA
        selected_tools = ["input_validator"]
        reason = "The question asks about observation modality; answer from ingestion metadata without model inference."
    elif request.input_mode == InputMode.CROSS_MODAL and cross_modal_intent.category != QuestionCategory.UNSUPPORTED:
        task = TaskType.CROSS_MODAL_ANALYSIS
        selected_tools = ["input_validator", "cross_modal_optical_sar_analyzer"]
        reason = "The selected Optical + SAR workflow requests source-aware joint or complementary evidence."
    elif task == TaskType.SAR_WATER_SEGMENTATION:
