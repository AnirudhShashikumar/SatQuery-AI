...
def _read_tiff_preview(data: bytes, descriptor: RasterDescriptor, selected_indices: List[int]) -> np.ndarray:
    rasterio_preview = _read_tiff_preview_rasterio(data, descriptor, selected_indices)
    ...
    try:
        with tifffile.TiffFile(io.BytesIO(data)) as dataset:
            ...
            if len(pages) >= descriptor.band_count and all(pages[index].ndim == 2 for index in selected_indices):
                ...
                axes = series.axes
            normalized = _normalize_to_hwc(raster, axes)
            if normalized.shape[2] >= descriptor.band_count and normalized.shape[2] != len(selected_indices):
                normalized = normalized[:, :, selected_indices]
            return normalized
    except ImageIngestionError:
        raise
    except (tifffile.TiffFileError, ValueError, TypeError, IndexError, OSError) as error:
...
def _read_tiff_preview_rasterio(data: bytes, descriptor: RasterDescriptor, selected_indices: List[int]) -> Optional[np.ndarray]:
    ...
    try:
        with MemoryFile(data) as memory_file:
            with memory_file.open() as dataset:
                ...
                if dataset.count < 1 or any(index < 0 or index >= dataset.count for index in selected_indices):
                    return None
                ...
                raster = dataset.read(
                    indexes=[index + 1 for index in selected_indices],
...
def _render_preview(
...
) -> Tuple[Image.Image, List[str], List[str], str]:
    warnings: List[str] = []
    ...
    if sampled.shape[2] == 1:
        ...
        bands_used = selected_band_names[:1] or ["band_1"]
        representation = "percentile-stretched grayscale display representation"
        if descriptor.band_count > 3 and not selected_band_names:
            warnings.append("No supported RGB band mapping was found; a first-band grayscale preview was generated for inspection only.")
    elif sampled.shape[2] == 2:
        ...
        bands_used = selected_band_names[:2] or ["band_1", "band_2"]
        ...
        bands_used = selected_band_names[:3] or ["band_1", "band_2", "band_3"]
        representation = "percentile-stretched RGB display representation"
        if descriptor.band_count > 3:
            warnings.append(f"Visual bands were selected explicitly: {selection_reason}")
...
def select_visual_bands(descriptor: RasterDescriptor) -> Tuple[List[int], List[str], str]:
    """Select display/model bands without silently guessing an RGB mapping."""

    count = descriptor.band_count
    interpretations = [str(value or "").strip().lower() for value in descriptor.color_interpretation]
    descriptions = [str(value or "").strip() for value in descriptor.band_descriptions]
    available = [
        (descriptions[index] if index < len(descriptions) else "")
        or (interpretations[index] if index < len(interpretations) else "")
        or f"band_{index + 1}"
        for index in range(count)
    ]
    rgb_aliases = ({"red", "r"}, {"green", "g"}, {"blue", "b"})
    rgb_indices: List[int] = []
    for aliases in rgb_aliases:
        match = next((index for index, value in enumerate(interpretations) if value in aliases), None)
        if match is None:
            rgb_indices = []
            break
        rgb_indices.append(match)
    if len(set(rgb_indices)) == 3:
        return rgb_indices, [available[index] for index in rgb_indices], "color interpretation explicitly identifies red, green, and blue"

    normalized = [re.sub(r"[^a-z0-9]+", "", value.lower()) for value in descriptions]
    sentinel_indices: List[int] = []
    for band in ("b4", "b3", "b2"):
        match = next((index for index, value in enumerate(normalized) if value == band or value.startswith(band + "0")), None)
        if match is None:
            sentinel_indices = []
            break
        sentinel_indices.append(match)
    if len(set(sentinel_indices)) == 3:
        return sentinel_indices, [available[index] for index in sentinel_indices], "band descriptions identify Sentinel-2-like B4/B3/B2 true color"

    if count == 3:
        return [0, 1, 2], available[:3], "three-band input retained in declared source order; semantic RGB use still requires modality validation"
    if count == 2:
        return [0, 1], available[:2], "two scientific bands retained for a disclosed false-color preview"
    if count == 1:
        return [0], available[:1], "single scientific band rendered as grayscale"
    return [0], [], "no supported RGB metadata mapping was found; only a non-semantic first-band grayscale preview was generated"
...
def _preview_intensity_statistics(raster: np.ndarray) -> Optional[Tuple[bool, str]]:
    values = np.asarray(raster)
    if values.ndim != 3 or not np.issubdtype(values.dtype, np.number) or values.shape[2] < 1:
        return None
    selected = values[:, :, : min(3, values.shape[2])].astype(np.float32, copy=False)
    gray = selected.mean(axis=2)
    finite = np.isfinite(gray)
    if not finite.any():
        return None
    valid = gray[finite]
    dynamic_range = float(np.percentile(valid, 99) - np.percentile(valid, 1))
    mean = float(valid.mean())
    coefficient_of_variation = float(valid.std() / max(abs(mean), 1.0))
    differences: List[np.ndarray] = []
    if gray.shape[0] > 1:
        differences.append(np.abs(np.diff(gray, axis=0)).reshape(-1))
    if gray.shape[1] > 1:
        differences.append(np.abs(np.diff(gray, axis=1)).reshape(-1))
    finite_differences = np.concatenate(differences) if differences else np.empty(0, dtype=np.float32)
    finite_differences = finite_differences[np.isfinite(finite_differences)]
    roughness = (
        float(np.median(finite_differences)) / max(dynamic_range, 1.0)
        if finite_differences.size else 0.0
    )
    sar_like = dynamic_range >= 64.0 and coefficient_of_variation >= 0.14 and roughness >= 0.025
    metrics = (
        f"intensity range p1-p99={dynamic_range:.1f}, coefficient of variation={coefficient_of_variation:.3f}, "
        f"local roughness={roughness:.3f}"
    )
    return sar_like, metrics
...
def _detect_modality(
    ...
    sar_hint = bool(re.search(r"(^|[^a-z0-9])(sar|radar|sentinel[-_ ]?1|s1[ab]?|vv|vh)([^a-z0-9]|$)", f"{descriptions} {filename}"))
    optical_hint = bool(re.search(r"(^|[^a-z0-9])(optical|panchromatic|pan|true[-_ ]?color|rgb)([^a-z0-9]|$)", filename))
    ...
    if representation == RepresentationType.DISPLAY_PREVIEW and descriptor.band_count == 1:
        statistics = _preview_intensity_statistics(raster) if raster is not None else None
        if statistics is not None:
            sar_like_statistics, metrics = statistics
            if sar_hint and sar_like_statistics and not optical_hint:
                limitations.append("SAR-preview detection is based on display-image statistics and a secondary filename or metadata hint; it does not recover scientific SAR calibration.")
                return (
                    ImageModality.SAR_PREVIEW,
                    DetectionConfidence.MEDIUM,
                    f"One-channel display has SAR-like intensity statistics and a SAR metadata/filename hint; classified as SAR preview ({metrics}).",
                    limitations,
                )
            if optical_hint and not sar_like_statistics:
                return (
                    ImageModality.OPTICAL_GRAYSCALE,
                    DetectionConfidence.MEDIUM,
                    f"The one-channel display carries an optical/panchromatic filename hint; classified as optical grayscale ({metrics}).",
                    limitations,
                )
        reason = "A one-band PNG/JPEG is a grayscale display representation; its scientific modality is not encoded."
        if sar_hint:
            reason += " The filename suggests SAR, but the intensity statistics do not provide sufficient corroboration."
    ...
    if representation == RepresentationType.DISPLAY_PREVIEW and rgb_like and raster is not None:
        values = np.asarray(raster)
        if values.ndim == 3 and values.shape[2] >= 3 and np.issubdtype(values.dtype, np.number):
            rgb = np.asarray(values[:, :, :3], dtype=np.float32)
            finite = np.all(np.isfinite(rgb), axis=2)
            if finite.any():
                sampled = rgb[finite]
                luminance = sampled.mean(axis=1)
                dynamic_range = float(np.percentile(luminance, 99) - np.percentile(luminance, 1))
                channel_delta = float(np.percentile(sampled.max(axis=1) - sampled.min(axis=1), 99))
                near_grayscale = channel_delta <= max(3.0, dynamic_range * 0.015)
                if near_grayscale:
                    statistics = _preview_intensity_statistics(rgb)
                    sar_like_statistics, intensity_metrics = statistics or (False, "intensity statistics unavailable")
                    metrics = f"channel delta p99={channel_delta:.1f}, {intensity_metrics}"
                    if sar_hint and sar_like_statistics and not optical_hint:
                        limitations.append("SAR-preview detection is based on display-image statistics and a secondary filename or metadata hint; it does not recover scientific SAR calibration.")
                        return (
                            ImageModality.SAR_PREVIEW,
                            DetectionConfidence.MEDIUM,
                            "Three-channel image is near-grayscale with SAR-like intensity statistics and a SAR metadata/filename hint; classified as SAR preview "
                            f"({metrics}).",
                            limitations,
                        )
                    if optical_hint and not sar_like_statistics:
                        return (
                            ImageModality.OPTICAL_GRAYSCALE,
                            DetectionConfidence.MEDIUM,
                            f"The RGB display is near-grayscale and carries an optical/panchromatic filename hint; classified as optical grayscale ({metrics}).",
                            limitations,
                        )
                    limitations.append("Near-grayscale RGB pixels alone cannot distinguish SAR from optical grayscale or panchromatic imagery.")
                    return (
                        ImageModality.UNKNOWN,
                        DetectionConfidence.LOW,
                        f"The RGB display is near-grayscale, but corroborating SAR or optical metadata is insufficient ({metrics}).",
                        limitations,
                    )
        return ImageModality.OPTICAL_RGB, DetectionConfidence.HIGH, "The display image contains explicit RGB channels with normal chromatic variation.", limitations
    if representation == RepresentationType.DISPLAY_PREVIEW and rgb_like:
        return ImageModality.OPTICAL_RGB, DetectionConfidence.HIGH, "The display image contains explicit RGB channels; pixel statistics were unavailable.", limitations
...
async def ingest_upload(upload: UploadFile) -> IngestedImage:
    ...
    try:
        ...
        started = time.perf_counter()
        selected_indices, selected_band_names, selection_reason = select_visual_bands(descriptor)
        raster = _read_tiff_preview(data, descriptor, selected_indices) if detected == ImageFormat.TIFF else _read_standard_preview(data, descriptor)
        ...
        detected_modality, detection_confidence, detection_reason, modality_limitations = _detect_modality(
            descriptor, representation, original_name, raster
        )
        statistics = _band_statistics(raster, descriptor)
        preview, preview_warnings, bands_used, image_representation = _render_preview(
            raster,
            descriptor,
            selected_band_names,
            selection_reason,
        ...
        analysis_band_count = 1 if (
            detected_modality == ImageModality.SAR_PREVIEW
            and representation == RepresentationType.DISPLAY_PREVIEW
            and raster.shape[2] >= 3
        ) else min(3, raster.shape[2])
        analysis_raster = np.ascontiguousarray(raster[::analysis_stride, ::analysis_stride, :analysis_band_count])
        ...
        metadata = ImageMetadata(
            ...
            input_band_count=descriptor.band_count,
            available_band_names=[
                ((descriptor.band_descriptions[index] or "").strip() if index < len(descriptor.band_descriptions) else "")
                or (descriptor.color_interpretation[index] if index < len(descriptor.color_interpretation) else "")
                or f"band_{index + 1}"
                for index in range(descriptor.band_count)
            ],
            selected_visual_bands=selected_band_names,
            band_selection_reason=selection_reason,
