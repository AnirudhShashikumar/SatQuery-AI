...
import {
  Activity, BarChart3, BrainCircuit, Braces, FileOutput, GitCompareArrows, HelpCircle, History, Images,
...
export const navigationGroups: readonly NavigationGroup[] = [
  ...
  { label: "Analysis", items: [
    ...
    { href: "/assistant/compare", label: "Mission Comparison", icon: GitCompareArrows, expectedHeading: "Mission Comparison", pageFile: "app/assistant/compare/page.tsx" },
  ] },
...
export function Sidebar() {
  ...
  const readyCount = Object.values(health.data?.models ?? {}).filter(model => model?.available).length;
  const modelCount = Object.keys(health.data?.models ?? {}).length;
  ...
  return <>
    ...
    <aside ref={sidebarRef} id="satquery-navigation" aria-label="Primary navigation drawer" className={cn("sat-sidebar", mobileOpen && "sat-sidebar-open", collapsed && "sat-sidebar-collapsed")}>
      ...
      <nav className="sat-nav" aria-label="SatQuery AI navigation">
        {navigationGroups.map(group => <section className="sat-nav-group" key={group.label} aria-labelledby={`sat-nav-${group.label.toLowerCase().replaceAll(" ", "-")}`}>
          <p className="sat-nav-label" id={`sat-nav-${group.label.toLowerCase().replaceAll(" ", "-")}`}>{group.label}</p>
          {group.items.map(item => <SidebarLink key={`${group.label}-${item.label}`} item={item} active={isSidebarItemActive(path, item, currentSearch)}/>) }
        </section>)}
      </nav>
      <div className="sat-sidebar-bottom">
        <div className="sat-agent-status" title={modelCount ? `${readyCount} of ${modelCount} local models ready` : "Checking local services"}>
          <span className={cn("sat-agent-dot", isOnline && "sat-agent-dot-online")}/>
          <span><strong>{isOnline ? "SatQuery Agent Ready" : health.isLoading ? "Connecting to agent" : "Agent unavailable"}</strong><small>{modelCount ? `${readyCount}/${modelCount} services online` : "Local intelligence runtime"}</small></span>
        </div>
