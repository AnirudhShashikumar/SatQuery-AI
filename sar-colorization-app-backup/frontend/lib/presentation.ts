...
export const presentationSteps = [
  ...
  },
  {
    ...
    subtitle: "Compatible optical pairs combine a primary TTP learned mask with independent deterministic evidence.",
    points: ["TTP learned and deterministic masks", "Agreement, disagreement, area, and regions", "Provenance and automatic fallback remain visible"],
    icon: "change",
    presenterNote: {
      say: "SatQuery AI uses TTP as the primary learned change detector for compatible optical pairs. An independent deterministic engine produces a second evidence stream. Agreement and disagreement are shown transparently, and the system does not infer the semantic cause of change.",
      show: "Run the analysis explicitly, then show the TTP mask, deterministic mask, agreement map, change percentage, largest region, provenance, and compact execution trace.",
      watchFor: "TTP output is not ground truth; incompatible pairs are never silently aligned and service failure uses the disclosed deterministic fallback.",
    },
    linkedRoute: "/assistant",
