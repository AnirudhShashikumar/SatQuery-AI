...
export type ChangeEvidencePreview = { label: "Difference map" | "TTP learned mask" | "Deterministic mask" | "Agreement" | "Disagreement" | "TTP overlay" | "Binary mask" | "Overlay"; path: string };
...
export function changeEvidencePreviews(response: AgentResponse): ChangeEvidencePreview[] {
  ...
  const candidates: Array<ChangeEvidencePreview | null> = [
    previews.difference ? { label: "Difference map", path: previews.difference } : null,
    hybrid && previews.ttp_mask ? { label: "TTP learned mask", path: previews.ttp_mask } : null,
    hybrid && previews.deterministic_mask ? { label: "Deterministic mask", path: previews.deterministic_mask } : null,
    ...
    hybrid && previews.ttp_overlay ? { label: "TTP overlay", path: previews.ttp_overlay } : null,
