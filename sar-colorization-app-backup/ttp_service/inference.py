...
class TTPInferenceEngine:
    ...
    def predict(self, earlier_data: bytes, later_data: bytes, earlier_suffix: str, later_suffix: str) -> np.ndarray:
        ...
        prediction = _unwrap_prediction(result)
        if prediction is None:
            raise TTPInferenceError("TTP returned no binary prediction.")
        ...
        if prediction.ndim != 2 or not np.all(np.isfinite(prediction)):
            raise TTPInferenceError("TTP returned an invalid prediction shape or non-finite values.")
        unique = np.unique(prediction)
        if not set(unique.tolist()).issubset({0, 1, False, True}):
            if np.issubdtype(prediction.dtype, np.floating) and float(prediction.min()) >= 0 and float(prediction.max()) <= 1:
                prediction = prediction >= 0.5
            else:
                raise TTPInferenceError("TTP returned values outside the binary class set.")
        return prediction.astype(bool, copy=False)
