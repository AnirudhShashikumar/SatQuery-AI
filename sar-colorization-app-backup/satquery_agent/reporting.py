...
SCHEMA_VERSION = "1.0"
TOOL_VERSION = "satquery-sar-routing-1.1"
...
def _report_previews(response: Dict[str, Any]) -> List[Tuple[str, str]]:
    ...
    seen = set()
    return [(label, url) for label, url in preferred + _named_previews(response) if not (url in seen or seen.add(url))]
...
def build_report_document(response: AgentResponse, generated_at: str, artifact_names: Iterable[str]) -> Dict[str, Any]:
    safe_response = _sanitize(response.model_dump(mode="json"))
...
def _pdf_bytes(document: Dict[str, Any]) -> bytes:
    ...
    for label, metadata in (("Primary image", document["input_summary"]["primary"]), ("Secondary image", document["input_summary"]["secondary"])):
        if not metadata:
            continue
        pages.text(label, 26, True)
        for key in ("safe_name", "format", "width", "height", "band_count", "dtype", "crs", "is_georeferenced", "bounds"):
    ...
    if compatibility:
        ...
        for key in ("compatible", "alignment_level", "same_dimensions", "same_crs", "same_transform", "overlap_ratio", "resampling_required"):
    ...
    if provenance.get("ttp"):
        for name in ("status", "model", "architecture", "training_dataset", "checkpoint", "checkpoint_fingerprint", "device", "runtime_ms", "model_load_ms", "reused_model"):
            pages.key_value(f"TTP {name.replace('_', ' ')}".title(), provenance["ttp"].get(name))
        pages.text("TTP output is a model-generated binary change prediction and is not ground truth.", 20, False, (108, 77, 16), 14)
