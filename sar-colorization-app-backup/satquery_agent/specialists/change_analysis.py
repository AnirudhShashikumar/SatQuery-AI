...
def statistics_from_binary_mask(mask: np.ndarray, source_width: int, source_height: int) -> ChangeStatistics:
    ...
    if values.ndim != 2 or values.shape != (source_height, source_width):
        raise ChangeAnalysisError("INVALID_MASK_DIMENSIONS", "The learned mask does not match the source image dimensions.")
