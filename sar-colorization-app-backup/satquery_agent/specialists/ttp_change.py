...
def ttp_enabled() -> bool:
    return os.getenv("TTP_ENABLED", "false").strip().lower() in {"1", "true", "yes", "on"}
...
class TTPServiceClient:
    ...
    def health(self, force: bool = False) -> Dict[str, Any]:
        ...
        try:
            ...
            raise TTPClientError("TIMEOUT", "TTP health check timed out; deterministic fallback was used.") from error
        except requests.RequestException as error:
            self._metrics["failures"] += 1
            raise TTPClientError("UNAVAILABLE", "TTP is unavailable; deterministic fallback was used.") from error
        if response.status_code != 200:
            raise TTPClientError("UNAVAILABLE", "TTP is unavailable; deterministic fallback was used.")
        ...
        if any(payload.get(key) != value for key, value in required.items()):
            raise TTPClientError("UNHEALTHY", "TTP health or provenance validation failed; deterministic fallback was used.")
        with self._health_lock:
