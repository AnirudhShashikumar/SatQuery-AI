...
class SingleImageSarTests(unittest.TestCase):
    ...
    def test_one_band_png_is_ambiguous_not_optical(self):
        body = self.inspect(png_bytes(sar_preview()), "sar 2.png", "image/png")
    ...
    def test_ambiguous_grayscale_stops_before_specialist(self):
        response = self.analyze(sar_preview(), override="auto")
        self.assertEqual(response.status_code, 200, response.text)
        body = response.json()
    ...
    def test_three_band_png_stays_optical(self):
        rgb = np.dstack([sar_preview(), sar_preview(), sar_preview()])
        body = self.inspect(png_bytes(rgb), "optical.png", "image/png")
        self.assertEqual(body["metadata"]["auto_detected_modality"], "optical_rgb")
        self.assertFalse(body["requires_modality_confirmation"])
